    ...
    try {
      ...
      if (!campaign_name) {
        return res
          ...
          .json({
            error: {
              message: `Request body must include a 'campaign_name' value`,
            },
          });
      }
      errorMessage = "";
      errorMessage = helpers.validateMinStringLength(
        campaign_name,
        4,
        "campaign_name"
      );
      ...
      errorMessage = helpers.validateType(
        campaign_name,
        "string",
        "campaign_name"
      );
...
userCampaignsRouter
  ...
  .all(async (req, res, next) => {
    ...
    try {
      ...
      const campaign = await CampaignsService.getUserCampaignById(
        db,
        userId,
        campId
      );
  ...
  .patch(jsonParser, async (req, res, next) => {
    ...
    try {
      const numOfValues = Object.values(updateCampaignFields).filter(Boolean)
        .length;
      if (numOfValues === 0) {
        return res
          .status(400)
          .json({
            error: { message: `Request body must include a field to update` },
          });
      }
      let error = "";
      ...
      if (campaign_name) {
        ...
        error = helpers.validateMinStringLength(
          campaign_name,
          4,
          "campaign_name"
        );
      ...
      await CampaignsService.updateUserCampaign(
        db,
        userId,
        campId,
...
campaignsRouter.route("/").get(async (req, res, next) => {
  const db = req.app.get("db");
