...
authRouter
    .post('/login', jsonBodyParser, async (req, res, next) => {
        ...
        try {
            ...
            const dbUser = await AuthService.getUserWithUserName(db, loginUser.username)

            if(!dbUser) {
                return res
                    .status(400)
                    .json({error: 'Incorrect username or user_password'})
            }

            //compare user_password to password given
            const compareMatch = await AuthService.comparePasswords(loginUser.user_password, dbUser.user_password)
